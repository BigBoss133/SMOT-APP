...
import { Home, Upload, LoaderCircle, MessageSquareText, FileText, Settings } from 'lucide-react';
      ...
      <nav className="nav-stack" data-testid="main-navigation">
        {items.map((item) => {
          ...
          return (
            ...
            >
              <Icon size={17} />
              <span data-testid={`nav-label-${item.key.toLowerCase()}`}>{text[item.key]}</span>
            </Link>
          );
        })}
      </nav>

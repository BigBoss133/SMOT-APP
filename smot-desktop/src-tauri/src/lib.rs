...
pub fn validate_upload_file_path(path: &str) -> Result<(), String> {
    ...
    if RESERVED_WINDOWS_NAMES.contains(&stem.to_uppercase().as_str()) {
        return Err(format!(
            "Invalid filename (reserved Windows name): {}",
            path
        ));
    }
    Ok(())
...
async fn get_system_status(state: tauri::State<'_, AppState>) -> Result<SystemStatus, String> {
    ...
    let active_model = match crate::ollama::check_ollama_status(&state.client).await {
        status if status.running => status
            .models
            .first()
            .cloned()
...
async fn start_indexing(
    ...
    let document_ids = if payload.document_ids.is_empty() {
        None
    } else {
    ...
    Ok(StartIndexingResponse {
        job_id: "indexing-active".to_string(),
    })
}
    ...
    let (sources, context) = {
        ...
        let context = sources
            ...
            .map(|s| {
                format!(
                    "[Document: {} | Page: {}] {}",
                    s.document_name, s.page, s.snippet
                )
            })
            .collect::<Vec<_>>()
            .join("\n\n");
